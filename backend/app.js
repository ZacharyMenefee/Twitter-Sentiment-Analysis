...
function getTweets() {
  //  const searchObject = await twitterClient.v2.search('Ukraine  lang:en');
...
const textDocumentInputs = getTweets();

async function sentimentAnalysis(client) {
  const results = await client.analyzeSentiment(textDocumentInputs);
  const storedResults = [];
...
app.get("/find", async (request, response) => {
  const textDocumentInputs = getTweets();
  const results = await sentimentAnalysis(azureClient);
